...
func scanRowMussNotBeCalled(t *testing.T) func(row internal.Row) bool {
	return func(row internal.Row) bool {
		t.Helper()
		t.Fail()
		return false
	}
...
func TestTree_ScanASC(t *testing.T) {
	...
	t.Run("root is leaf with limit", func(t *testing.T) {
		...
		var callTimer int
		var actualRows []internal.Row

		assert.NoError(t, tree.ScanASC(func(row internal.Row) bool {
			callTimer++
			actualRows = append(actualRows, row)
			return callTimer == 2
		}))

		assert.Equal(t, []internal.Row{"row 1", "row 2"}, actualRows)
	})
	...
	t.Run("root is not leaf without limit", func(t *testing.T) {
		...
		assert.Equal(t, []internal.Row{"row 1", "row 2", "row 3", "row 4"}, actualRows)
	})
	...
	t.Run("root is not leaf fail to load right", func(t *testing.T) {
		...
		assert.Equal(t, err, tree.ScanASC(func(row internal.Row) bool {
			actualRows = append(actualRows, row)
			return false
		}))
...
func TestTree_ScanDESC(t *testing.T) {
	...
	t.Run("root is leaf with limit", func(t *testing.T) {
		...
		assert.NoError(t, tree.ScanDESC(func(row internal.Row) bool {
			callTimer++
			actualRows = append(actualRows, row)
			return callTimer == 2
		}))

		assert.Equal(t, []internal.Row{"row 3", "row 2"}, actualRows)
	})
	...
	t.Run("root is not leaf without limit", func(t *testing.T) {
		...
		assert.Equal(t, []internal.Row{"row 4", "row 3", "row 2", "row 1"}, actualRows)
	})
	...
	t.Run("root is not leaf fail to load left", func(t *testing.T) {
		...
		assert.Equal(t, err, tree.ScanDESC(func(row internal.Row) bool {
			actualRows = append(actualRows, row)
			return false
		}))

		assert.Equal(t, []internal.Row{"row 4", "row 3"}, actualRows)
	})
