...
func TestStorage_Begin(t *testing.T) {
	...
	storage := NewStorage(txManager, txLocks)
...
func TestStorage_Get(t *testing.T) {
	...
	t.Run("not found", func(t *testing.T) {
		txObj := NewTxObj(1)
		txManager.EXPECT().Begin().Return(txObj)
		txManager.EXPECT().Get(txObj, base.IntKey(123)).Return(nil, nil)
		txManager.EXPECT().Rollback(txObj).Return(nil)
		txLocks.EXPECT().Release(txObj.GetID())

		row, err := storage.Get(base.IntKey(123))
		assert.Nil(t, row)
	...
	t.Run("error on txManager read", func(t *testing.T) {
		...
		originErr := errors.New("some error")
		txManager.EXPECT().Begin().Return(txObj)
		txManager.EXPECT().Get(txObj, base.IntKey(123)).Return(nil, originErr)
		txManager.EXPECT().Rollback(txObj).Return(nil)
		txLocks.EXPECT().Release(txObj.GetID())
	...
	t.Run("found in txManager read", func(t *testing.T) {
		txObj := NewTxObj(123)
		txManager.EXPECT().Begin().Return(txObj)
		txManager.EXPECT().Get(txObj, base.IntKey(123)).Return("some value", nil)
		txManager.EXPECT().Rollback(txObj).Return(nil)
		txLocks.EXPECT().Release(txObj.GetID())
...
func TestStorage_Set(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		txObj := NewTxObj(1)
		txManager.EXPECT().Begin().Return(txObj)
		txManager.EXPECT().Set(txObj, base.IntKey(123), "some value").Return(nil)
		txManager.EXPECT().Commit(txObj).Return(nil)
		txLocks.EXPECT().LockKey(txObj.GetID(), false, base.IntKey(123))
		txLocks.EXPECT().Release(txObj.GetID())

		err := storage.Set(base.IntKey(123), "some value")
		assert.NoError(t, err)
	})
	...
	t.Run("error on get row", func(t *testing.T) {
		...
		originErr := errors.New("some error")
		txManager.EXPECT().Begin().Return(txObj)
		txManager.EXPECT().Set(txObj, base.IntKey(123), "some value").Return(originErr)
		txManager.EXPECT().Rollback(txObj).Return(nil)
		txLocks.EXPECT().LockKey(txObj.GetID(), false, base.IntKey(123))
		txLocks.EXPECT().Release(txObj.GetID())
	...
	t.Run("error on lock", func(t *testing.T) {
		...
		txManager.EXPECT().Begin().Return(txObj)
		txManager.EXPECT().Rollback(txObj).Return(nil)
		txLocks.EXPECT().LockKey(txObj.GetID(), false, base.IntKey(123)).Return(originErr)
		txLocks.EXPECT().Release(txObj.GetID())
...
func TestStorage_LockKey(t *testing.T) {
	...
	txLocks := NewMockLocks(ctrl)
	storage := NewStorage(NewMockTxManager(ctrl), txLocks)
...
func TestStorage_LockKeys(t *testing.T) {
	...
	t.Run("success lock", func(t *testing.T) {
		...
		assert.NoError(t, storage.LockKeys(tx, []base.Key{key1, key2}))
	})
	...
	t.Run("fail lock", func(t *testing.T) {
		...
		actualErr := storage.LockKeys(tx, []base.Key{key1, key2})
		assert.Error(t, actualErr)
		assert.Equal(t, err, actualErr)
	})
}
